...
class DataManagerController extends ProjectBackendController {
    ...
    public function iframeManagerAction() {
        ...
        ), 'setup/problem/iframe/data_manager.php');
    }
    ...
    public function ajaxRemoveAction() {
        ...
        if ($problemInfo['user_id'] != $this->loginUserInfo['id']) {
            $this->renderError('您没有权限操作！');
        }
        if (!$problemInfo['hidden']) {
            $this->renderError('题目已经公开，您无法操作！');
        }
        ...
        $history = "<p>{$dataTime} 用户{$this->loginUserInfo['username']}删除文件{$file}</p>";
    ...
    public function ajaxCopyAction() {
        ...
        $history = "<p>{$dateTime} 用户{$this->loginUserInfo['username']}复制文件{$file}</p>";
        OjProblemInterface::auditHistory(array(
            'problem_id'     => $problemId,
            'append_history' => $history,
        ));
    ...
    public function ajaxUploadAction() {
        ...
        if ($problemInfo['user_id'] != $this->loginUserInfo['id']) {
            $this->echoJson(1, '您没有权限操作！');
        }
        if (!$problemInfo['hidden']) {
            $this->echoJson(1, '题目已经公开，您没有权限操作！');
        }
        ...
        // 记录操作
        $dateTime = date('Y-m-d H:i:s', time());
        $history = "<p>{$dateTime} 用户{$this->loginUserInfo['username']}批量上传了文件</p>";
        OjProblemInterface::auditHistory(array(
            'problem_id'     => $problemId,
            'append_history' => $history,
        ));
