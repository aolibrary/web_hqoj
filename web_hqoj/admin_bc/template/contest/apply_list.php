...
<table class="mt10 mb10 f12 widget-table widget-table-hover">
    ...
    <?php   foreach ($this->applyList as $applyInfo) {
        ...
        <tr>
            ...
            <td>
                <p>竞赛ID：<?php echo $contestInfo['id']; ?>&nbsp;&nbsp;<a href="/contest_detail/?contest-id=<?php echo $contestInfo['id']; ?>"><?php echo $contestInfo['title']; ?></a></p>
...
<script>
    seajs.use(['jquery', 'layer', 'notice'], function($, layer, notice) {
        ...
        $('a[name=change-status]').click(function(e) {
            e.preventDefault();
            $.ajax({
                url: '/contest_applyList/ajaxChangeStatus/',
                type: 'post',
                dataType: 'json',
                data: {
