...
void writeLog(const char *fmt, ...) {
    ...
    char buffer[4096];
    sprintf(buffer, "%s/log/daemon.log", ojHome);
...
int getJobs(int *jobs) {
    ...
    char sql[BUF_SIZE];
    sprintf(sql
        , "SELECT id FROM oj_judge WHERE result<2 ORDER BY result ASC, id ASC limit %d"
        , MAX_RUNNING * 2);
    if (mysql_real_query(conn, sql, strlen(sql))) {
...
bool updateSolution(int queueId, int result) {
    ...
    char sql[BUF_SIZE];
    sprintf(sql
        , "UPDATE oj_judge SET result=%d,time_cost=0,memory_cost=0,judge_time=%ld,ce='',re='',detail='' WHERE id=%d AND result<2 LIMIT 1"
